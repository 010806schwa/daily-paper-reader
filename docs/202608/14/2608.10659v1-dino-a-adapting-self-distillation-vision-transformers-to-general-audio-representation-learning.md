...
score: 6.0
evidence: 通用音频表示学习方法可直接应用于舰船噪声等声学分类任务
tldr: 自监督视觉范式DINO尚未被用于一般音频分类。本文提出DINO-A，保留其多裁剪、EMA教师和高维投影，将输入换成log-mel谱图，在FSD50K预训练ViT与CNN。线性探针下小patch全任务领先；卷积胜语音、ViT胜环境声与音乐；与BYOL-A v2差11.96个百分点，源于高维投影与FSD50K规模不匹配及多裁剪开销。贡献在于揭示DINO迁移到音频小数据时的适用边界。
source: arxiv
...
motivation: 尚无工作将经典DINO范式迁移至一般音频分类，本文填补该空白并分析其与BYOL-A v2的差距来源。
method: 保留DINO的多裁剪、EMA教师与高维投影，仅替换输入为log-mel谱图和BYOL-A v2增强，在FSD50K预训练三个骨干网络。
result: 小patch全面领先；卷积胜语音、ViT胜环境声与音乐；与BYOL-A v2平均差11.96个百分点，源于高维投影与数据量不匹配及多裁剪开销。
conclusion: 高维投影空间在视觉中有效，但在小规模音频数据上成为负担，DINO-A揭示了跨模态适配的关键约束。
---

## 摘要
我们提出了DINO-A，一种从视觉到通用音频表示学习的自蒸馏适配方法。虽然DINO已成为自监督视觉中的经典方法，且先前的音频工作已探索了潜在预测（BYOL-A）和掩码建模（Audio-MAE、BEATs），但尚无先前工作以BYOL-A引入BYOL的方式将经典DINO引入通用音频分类。DINO-A保留了DINO的多裁剪、EMA教师和高维投影，仅将输入模态和增强替换为对数梅尔频谱图和BYOL-A v2增强块。我们在FSD50K上预训练了三个骨干网络——两个具有8x8和16x16 patch的视觉变换器以及一个卷积编码器——并通过线性探测在ESC-50、Speech Commands v2、UrbanSound8K和GTZAN上评估它们。三个发现刻画了所得到的表示。视觉变换器家族内部的patch分辨率对表示质量具有一致的影响，较小的patch在所有四个任务中胜出。视觉变换器与卷积骨干的选择与任务类型相互作用：卷积网络在语音上领先，而视觉变换器在环境声音和音乐上领先。在相同的预训练和评估条件下，DINO-A和BYOL-A v2平均相差11.96个百分点，我们将这一差异归因于两个机制：DINO的高维投影空间与FSD50K有限规模之间的相互作用，以及DINO使用但BYOL-A v2未使用的多裁剪增强的额外成本。高维投影空间是DINO在视觉中成功的关键，但在FSD50K规模下却成为负担。
